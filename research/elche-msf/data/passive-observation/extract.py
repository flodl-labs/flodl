#!/usr/bin/env python3
"""Extract paper-evidence artifacts from the passive-observation sweep.

Cell shape: 5 seeds (0-4) x 2 modes (cpu-async, nccl-async) x 2 guards
(msf, trend) = 20 cells. Cell name encodes mode and guard:
`seed-{S}-{mode}-{guard}`.
...
DEFAULT_RAW = REPO_ROOT / "ddp-bench" / "runs" / "overnight-2026-05-04"
...
def cell_to_mode(cell: str) -> str:
    """Recover the bench mode from the cell name (used as an inner subdir)."""
    if "cpu-async" in cell:
        return "cpu-async"
    if "nccl-async" in cell:
        return "nccl-async"
    raise ValueError(f"unknown mode in cell name: {cell}")


def extract_cell(raw_root: Path, out_root: Path, cell: str) -> bool:
    raw_cell = raw_root / cell
    raw_inner = raw_cell / "resnet-graph" / cell_to_mode(cell)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Passive-observation sweep extractor.")
    parser.add_argument(
        "--raw-base",
        type=Path,
        default=DEFAULT_RAW,
