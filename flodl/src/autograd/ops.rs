...
/// Normalize a possibly-negative dimension index to a valid usize.
fn normalize_dim(dim: i32, ndim: usize) -> usize {
    if dim < 0 {
...
impl Variable {
    ...
    pub fn div_scalar(&self, scalar: f64) -> Result<Variable> {
        let result = self.inner.borrow().data.div_scalar(scalar)?;
        ...
        let grad_fn = GradFn {
            name: "DivScalarBackward",
            inputs: vec![self.clone()],
            apply: Box::new(move |grad: &Tensor| Ok(vec![grad.div_scalar(scalar)?])),
        };
    ...
    pub fn mean(&self) -> Result<Variable> {
        ...
        let numel = self.inner.borrow().data.numel() as f64;
        let input_shape = self.inner.borrow().data.shape();
        let input_dtype = self.inner.borrow().data.dtype();
        let input_device = self.inner.borrow().data.device();
        ...
        let grad_fn = GradFn {
            ...
            apply: Box::new(move |grad: &Tensor| {
                ...
                let ones = Tensor::ones(&input_shape, opts)?;
                Ok(vec![ones.mul_scalar(grad.item()? / numel)?])
            }),
        };
    ...
    pub fn select(&self, dim: i32, index: i64) -> Result<Variable> {
        ...
        let full_shape = self.inner.borrow().data.shape();
        let d = normalize_dim(dim, full_shape.len());
    ...
    pub fn cat(&self, other: &Variable, dim: i32) -> Result<Variable> {
        ...
        let a_shape = self.inner.borrow().data.shape();
        let d = normalize_dim(dim, a_shape.len());
        let a_size = a_shape[d];
        let grad_fn = GradFn {
            name: "CatBackward",
            inputs: vec![self.clone(), other.clone()],
            apply: Box::new(move |grad: &Tensor| {
                let ga = grad.narrow(dim, 0, a_size)?;
                let gb = grad.narrow(dim, a_size, grad.shape()[d] - a_size)?;
                Ok(vec![ga, gb])
            }),
        };
    ...
    /// Walks the computation graph in reverse topological order, accumulating
    /// gradients at each leaf variable. Saved tensors are freed as each node
    /// is processed — deterministic VRAM release.
    ///
    /// ```ignore
    /// let loss = pred.sub(&target)?.pow_scalar(2.0)?.mean()?;
    /// loss.backward()?;
