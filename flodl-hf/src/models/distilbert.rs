...
use crate::models::bert::{build_extended_attention_mask, Answer};
...
impl DistilBertConfig {
    ...
    pub fn from_json_str(s: &str) -> Result<Self> {
        ...
        Ok(DistilBertConfig {
            ...
            dim:                     get_i64("dim")?,
            n_layers:                get_i64("n_layers")?,
            n_heads:                 get_i64("n_heads")?,
            hidden_dim:              get_i64("hidden_dim")?,
            max_position_embeddings: get_i64("max_position_embeddings")?,
            pad_token_id:            get_i64_or("pad_token_id", 0),
            dropout:                 get_f64_or("dropout", 0.1),
            attention_dropout:       get_f64_or("attention_dropout", 0.1),
            qa_dropout:              get_f64_or("qa_dropout", 0.1),
            seq_classif_dropout:     get_f64_or("seq_classif_dropout", 0.2),
            sinusoidal_pos_embds:    get_bool_or("sinusoidal_pos_embds", false),
...
/// One labelled token inside a [`DistilBertForTokenClassification`]
/// prediction. Same shape as
/// [`crate::models::bert::TokenPrediction`] (duplicated here so the
/// DistilBERT path doesn't force callers to import from `bert`).
#[cfg(feature = "tokenizer")]
#[derive(Debug, Clone)]
