#!/usr/bin/env python3
"""Generate the parity fixture for `AlbertForTokenClassification`.

Runs HuggingFace `AlbertForTokenClassification` on a pinned sentence and
writes inputs + per-token reference logits to
`flodl-hf/tests/fixtures/albert_tokencls_parity.safetensors`.

Fixture model: `ArBert/albert-base-v2-finetuned-ner` (CoNLL-2003 NER).

Run via `fdl flodl-hf parity albert-tokencls`.
"""
...
MODEL_ID = "ArBert/albert-base-v2-finetuned-ner"
REVISION: str | None = None

FIXTURE_PATH = (
    Path(__file__).resolve().parents[1]
    / "tests" / "fixtures" / "albert_tokencls_parity.safetensors"
)
...
def main() -> None:
    ...
    model = AlbertForTokenClassification.from_pretrained(MODEL_ID, revision=sha).eval()
