#!/usr/bin/env python3
"""Generate the parity fixture for `AlbertForSequenceClassification`.

Runs HuggingFace `AlbertForSequenceClassification` on a pinned text input
and writes both the tokenised inputs and the reference logits to
`flodl-hf/tests/fixtures/albert_seqcls_parity.safetensors`.

Fixture model: `bhadresh-savani/albert-base-v2-emotion` (6 emotion classes).

Run via `fdl flodl-hf parity albert-seqcls`.
"""
...
MODEL_ID = "bhadresh-savani/albert-base-v2-emotion"
REVISION: str | None = None

FIXTURE_PATH = (
    Path(__file__).resolve().parents[1]
    / "tests" / "fixtures" / "albert_seqcls_parity.safetensors"
)

PROMPT = "I love this framework so much"
...
def main() -> None:
    ...
    model = AlbertForSequenceClassification.from_pretrained(MODEL_ID, revision=sha).eval()
