#!/usr/bin/env python3
"""Generate the parity fixture for `DebertaV2ForQuestionAnswering`.

Runs HuggingFace `DebertaV2ForQuestionAnswering` on a pinned
(question, context) pair and writes inputs + `[B, S, 2]` start/end
logits to `flodl-hf/tests/fixtures/deberta_v2_qa_parity.safetensors`.

Fixture model: `deepset/deberta-v3-base-squad2` — DeBERTa-v3-base
fine-tune on SQuAD v2. 2-input convention: `input_ids` +
`attention_mask` (no `token_type_ids`).

Run via `fdl flodl-hf parity deberta-v2-qa`.
"""
...
FIXTURE_PATH = (
    ...
    / "tests" / "fixtures" / "deberta_v2_qa_parity.safetensors"
)
