#!/usr/bin/env python3
"""Generate the parity fixture for `AlbertForQuestionAnswering`.

Runs HuggingFace `AlbertForQuestionAnswering` on a pinned (question, context)
pair and writes inputs + start/end logits to
`flodl-hf/tests/fixtures/albert_qa_parity.safetensors`.

Fixture model: `twmkn9/albert-base-v2-squad2` (SQuAD v2, 2-wide head).

Run via `fdl flodl-hf parity albert-qa`.
"""
...
FIXTURE_PATH = (
    ...
    / "tests" / "fixtures" / "albert_qa_parity.safetensors"
)
...
def main() -> None:
    ...
    model = AlbertForQuestionAnswering.from_pretrained(MODEL_ID, revision=sha).eval()
