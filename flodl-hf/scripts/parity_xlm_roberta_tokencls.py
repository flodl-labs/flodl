#!/usr/bin/env python3
"""Generate the parity fixture for `XLMRobertaForTokenClassification`.

Runs HuggingFace `XLMRobertaForTokenClassification` on a pinned sentence
and writes inputs + per-token reference logits to
`flodl-hf/tests/fixtures/xlm_roberta_tokencls_parity.safetensors`.

Fixture model: `Davlan/xlm-roberta-large-ner-hrl` — multilingual NER
fine-tune over high-resource languages. Larger checkpoint (~2.2 GB)
shared with the export-matrix runner so the HF cache is reused
across `fdl test-live` and `fdl flodl-hf verify-matrix` invocations.

Run via `fdl flodl-hf parity xlm-roberta-tokencls`.
"""
...
FIXTURE_PATH = (
    ...
    / "tests" / "fixtures" / "xlm_roberta_tokencls_parity.safetensors"
)
