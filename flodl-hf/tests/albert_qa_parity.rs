//! PyTorch parity for `AlbertForQuestionAnswering::from_pretrained`.
//!
//! Loads the fixture generated by `flodl-hf/scripts/parity_albert_qa.py`
//! (`fdl flodl-hf parity albert-qa`) and compares `[B, S, 2]` logits
//! against the HF Python reference.
//!
//! `_live` — pulls the fine-tuned SQuAD weights from the Hub. Run with
//! `fdl test-live`.
...
use flodl_hf::models::albert::AlbertForQuestionAnswering;
use flodl_hf::models::bert::build_extended_attention_mask;

const FIXTURE: &str = "tests/fixtures/albert_qa_parity.safetensors";
const MODEL_ID: &str = "twmkn9/albert-base-v2-squad2";
...
fn albert_qa_parity_vs_pytorch_live() {
    let dev = Device::CPU;

    let bytes = std::fs::read(Path::new(FIXTURE))
        .unwrap_or_else(|e| panic!("reading {FIXTURE}: {e} (run `fdl flodl-hf parity albert-qa` to regenerate)"));
    ...
    let head = AlbertForQuestionAnswering::from_pretrained(MODEL_ID).unwrap();
    head.graph().eval();
