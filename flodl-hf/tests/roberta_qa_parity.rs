//! PyTorch parity for `RobertaForQuestionAnswering::from_pretrained`.
//!
//! Loads the fixture generated by
//! `flodl-hf/scripts/parity_roberta_qa.py`
//! (`fdl flodl-hf parity roberta-qa`) and compares `[B, S, 2]` logits
//! against the HF Python reference on the pinned (question, context)
//! pair.
//!
//! `_live` — pulls the SQuAD-fine-tuned weights from the Hub. Run
//! with `fdl test-live`.
...
const FIXTURE: &str = "tests/fixtures/roberta_qa_parity.safetensors";
const MODEL_ID: &str = "deepset/roberta-base-squad2";
...
fn roberta_qa_parity_vs_pytorch_live() {
    let dev = Device::CPU;

    let bytes = std::fs::read(Path::new(FIXTURE))
        .unwrap_or_else(|e| panic!("reading {FIXTURE}: {e} (run `fdl flodl-hf parity roberta-qa` to regenerate)"));
    ...
    let head = RobertaForQuestionAnswering::from_pretrained(MODEL_ID).unwrap();
    head.graph().eval();
