//! PyTorch parity for `AlbertForTokenClassification::from_pretrained`.
//!
//! Loads the fixture generated by `flodl-hf/scripts/parity_albert_tokencls.py`
//! (`fdl flodl-hf parity albert-tokencls`) and compares per-token logits
//! against the HF Python reference.
//!
//! `_live` — pulls the fine-tuned NER weights from the Hub. Run with
//! `fdl test-live`.
...
use flodl_hf::models::albert::AlbertForTokenClassification;
use flodl_hf::models::bert::build_extended_attention_mask;

const FIXTURE: &str = "tests/fixtures/albert_tokencls_parity.safetensors";
const MODEL_ID: &str = "ArBert/albert-base-v2-finetuned-ner";
...
fn albert_tokencls_parity_vs_pytorch_live() {
    let dev = Device::CPU;

    let bytes = std::fs::read(Path::new(FIXTURE))
        .unwrap_or_else(|e| panic!("reading {FIXTURE}: {e} (run `fdl flodl-hf parity albert-tokencls` to regenerate)"));
    ...
    let head = AlbertForTokenClassification::from_pretrained(MODEL_ID).unwrap();
    head.graph().eval();
