//! PyTorch parity for `AlbertForSequenceClassification::from_pretrained`.
//!
//! Loads the fixture generated by `flodl-hf/scripts/parity_albert_seqcls.py`
//! (`fdl flodl-hf parity albert-seqcls`) and compares flodl's classifier
...
use flodl_hf::models::albert::AlbertForSequenceClassification;
use flodl_hf::models::bert::build_extended_attention_mask;

const FIXTURE: &str = "tests/fixtures/albert_seqcls_parity.safetensors";
const MODEL_ID: &str = "bhadresh-savani/albert-base-v2-emotion";

/// Same 1e-5 budget as the base-ALBERT parity test — classifier head is
...
fn albert_seqcls_parity_vs_pytorch_live() {
    let dev = Device::CPU;

    let bytes = std::fs::read(Path::new(FIXTURE))
        .unwrap_or_else(|e| panic!("reading {FIXTURE}: {e} (run `fdl flodl-hf parity albert-seqcls` to regenerate)"));
    ...
    let head = AlbertForSequenceClassification::from_pretrained(MODEL_ID).unwrap();
    head.graph().eval();
