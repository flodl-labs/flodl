//! PyTorch parity for `BertForMaskedLM::from_pretrained`.
//!
//! Loads the fixture generated by `flodl-hf/scripts/parity_bert_mlm.py`
//! (`fdl flodl-hf parity bert-mlm`) and compares per-position
//! vocabulary logits against the HF Python reference.
//!
//! Verifies end-to-end that the weight-tied decoder produces the same
//! logits as HF's `tie_weights()`-backed runtime: the shared
//! `bert.embeddings.word_embeddings.weight` tensor drives both the
...
use flodl_hf::models::bert::{build_extended_attention_mask, BertForMaskedLM};
...
// to 1e-4. Still tight enough to catch any real disagreement in the
// tied-decoder wiring.
...
fn bert_mlm_parity_vs_pytorch_live() {
    let dev = Device::CPU;

    let bytes = std::fs::read(Path::new(FIXTURE))
        .unwrap_or_else(|e| panic!("reading {FIXTURE}: {e} (run `fdl flodl-hf parity bert-mlm` to regenerate)"));
    ...
    let head = BertForMaskedLM::from_pretrained(MODEL_ID).unwrap();
    head.graph().eval();
