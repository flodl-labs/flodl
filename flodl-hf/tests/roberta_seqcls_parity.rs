//! PyTorch parity for `RobertaForSequenceClassification::from_pretrained`.
//!
//! Loads the fixture generated by
//! `flodl-hf/scripts/parity_roberta_seqcls.py`
//! (`fdl flodl-hf parity roberta-seqcls`) and compares flodl's
//! classifier logits against the HF Python reference on the pinned
//! prompt.
//!
//! `_live` — pulls the fine-tuned sentiment weights from the Hub. Run
//! with `fdl test-live`.
...
const FIXTURE: &str = "tests/fixtures/roberta_seqcls_parity.safetensors";
...
fn roberta_seqcls_parity_vs_pytorch_live() {
    let dev = Device::CPU;

    let bytes = std::fs::read(Path::new(FIXTURE))
        .unwrap_or_else(|e| panic!("reading {FIXTURE}: {e} (run `fdl flodl-hf parity roberta-seqcls` to regenerate)"));
